...
import { Lock, Ban, UserCheck } from 'lucide-react';
...
export default function UserDetailPage() {
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            alert("Security status and profile updated!");
        } catch (err) {
            ...
            <div className="bg-white rounded-[40px] shadow-sm border border-gray-100 overflow-hidden">
                ...
                <div className="px-10 pb-10 -mt-12 space-y-10">
                    <div className="flex flex-col md:flex-row md:items-end gap-6">
                        ...
                        {currentUser?.role === 'super_admin' && id !== currentUser.uid && (
                            <div className="flex flex-col items-end gap-2">
                                <span className={`px-4 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest ${userData.status === 'revoked' ? 'bg-rose-100 text-rose-600' : 'bg-emerald-100 text-emerald-600'}`}>
                                    Status: {userData.status || 'active'}
                                </span>
                                <button
                                    onClick={() => setUserData({ ...userData, status: userData.status === 'revoked' ? 'active' : 'revoked' })}
                                    className={`flex items-center gap-2 px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest border-2 transition-all ${userData.status === 'revoked' ? 'bg-emerald-500 border-emerald-500 text-white' : 'bg-white border-rose-200 text-rose-500 hover:bg-rose-50'}`}
                                >
                                    {userData.status === 'revoked' ? <UserCheck size={14} /> : <Ban size={14} />}
                                    {userData.status === 'revoked' ? 'Restore Access' : 'Revoke Access'}
                                </button>
                            </div>
                        )}
                    </div>
                    ...
                    <div className="md:col-span-2 space-y-6 pt-6 mt-6 border-t border-gray-100">
                        <h3 className="text-[10px] font-black uppercase tracking-[0.2em] text-[#6c757d]">Security Credentials</h3>
                        <div className="bg-gray-50 rounded-[32px] p-8 grid grid-cols-1 md:grid-cols-2 gap-8">
                            <div className="space-y-2">
                                <label className="text-[10px] font-black text-[#6c757d] uppercase px-1">Update Access Password</label>
                                <div className="relative">
                                    ...
                                    <input
                                        ...
                                        className="w-full bg-white p-4 pl-12 rounded-2xl border-2 border-transparent focus:border-[#107d92] outline-none font-black tracking-widest text-[#107d92]"
                                        placeholder="8+ mixed characters"
                                    />
                                </div>
                                <PasswordStrength password={userData.password || ''} />
                            </div>
                            <div className="flex flex-col justify-center text-[10px] font-bold text-[#6c757d] space-y-2 bg-white/50 p-6 rounded-2xl border border-gray-100/50">
                                <p className="uppercase tracking-widest text-[#107d92]">Security Policy:</p>
                                <p>• Password must be unique and complex.</p>
                                <p>• Changes will require immediate re-login.</p>
